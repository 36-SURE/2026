...
anscombe |> 
  ...
  summarise(across(c(x, y), list(mean = mean, var = var)),
            x_y_cor = cor(x, y)) |>
  mutate(y_mean = round(y_mean, 3),
         y_var = round(y_var, 3), 
         x_y_cor = round(x_y_cor, 3)) |>
  gt()
```
:::
:::
...
datasauRus::datasaurus_dozen |> 
  ...
  summarise(across(c(x, y), list(mean = mean, var = var)),
            x_y_cor = cor(x, y)) |> 
  mutate(x_mean = round(x_mean, 1), 
         x_var = round(x_var, 1), 
         y_mean = round(y_mean, 1),
         y_var = round(y_var, 1), 
         x_y_cor = round(x_y_cor, 2)) |>
  gt()
ggplot2::theme_set(ggplot2::theme_gray(base_size = 20))
```
:::
...
- At any time, you can use a typical `theme()` call to override these choices. 

```{r}
theme_set(theme_bw()) # baseline theme (we like theme_bw())
...
*   [Nicola Rennie's TidyTuesday Github](https://github.com/nrennie)

*   [Erin's random data visualization code bits](https://efranke22.github.io/rtricks/#data-visualization)
<!-- ::: columns -->
